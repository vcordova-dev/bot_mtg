...
def handle_submit():
    ...
    send_to_google_sheets(selected_option, input_text)

# Função para enviar dados para o Google Sheets
def send_to_google_sheets(selected_option, input_text):
    ...
    worksheet = gc.open('Nome da Sua Planilha').sheet1  # Substitua pelo nome da sua planilha
    
    # Preparar dados
    entry_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    row = [selected_option, input_text, entry_time]
    
    # Adicionar linha à planilha
    worksheet.append_row(row)
    print(f"Dados enviados para o Google Sheets: {row}")
